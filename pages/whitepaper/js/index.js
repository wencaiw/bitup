(function(){
    ...
    document.getElementById('lang-btn').onclick = function(){
      if(this.innerText.indexOf('English') != -1){
          window.location.href = 'http://'+location.host + location.pathname
      }else{
          window.location.href = 'http://'+location.host + location.pathname + '?lang=cn'
      }
    }

    //关闭糖果浮层
    //document.getElementById('candyClose').onclick = function(event){
    //  event.preventDefault();
    //  document.getElementById('candy-masker').style.display = "none";
    //}

    //显示隐藏导航
    document.querySelector(".nav-bar .nav").onclick = function(event){
      event = event || window.event;
      event.stopPropagation();
      var navList = document.querySelector(".nav-bar .nav-list");
    ...
    document.querySelector("body").onclick = function(){
      document.querySelector(".nav-bar .nav-list").style.display = 'none';
    }
    //显示隐藏更多媒体
    document.querySelector(".report-list .report-more-icon").onclick = function(){
      var moreList = document.getElementById("moreList");
      var moreIcon = document.querySelector(".report-list .report-more-icon");
      if(moreList.style.display == 'none'){
        moreList.style.display = 'block';
        moreIcon.src = moreIcon.src.replace("icon-arrow-down.png","icon-arrow-up.png");
      }
      else {
        moreList.style.display = 'none';
        moreIcon.src = moreIcon.src.replace("icon-arrow-up.png","icon-arrow-down.png");
